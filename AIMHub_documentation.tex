...
% Markers used in contributed sections to flag changes for review. They are
% rendered as plain text; redefine them to highlight changes when reviewing.
\providecommand{\changed}[1]{#1}
\newenvironment{changedblock}{}{}

% \AIMHubVersion is defined in version.tex, which tex.sh regenerates from the
% AIM/Hub model version (../define/version.gms). Do not edit the title by hand.
\input{version}
\title{AIM/Hub \AIMHubVersion{} Documentation}
...
\\
  ...
  \item September, 2026 S. Fujimori Version 3.0.0 \\
      From this version on, the documentation carries the version of the
      AIM/Hub model it describes (shown in the title), and changes are
      recorded in CHANGELOG.md of the AIM/Hub repository.
...
\item[set magicc=off]deterministic MAGICC run (tools/MAGICC), writing the global-mean temperature to output/magicc/iiasa\_database\_climate.gdx.
\item[set climate\_assessment=off]probabilistic MAGICC 7.5.3 ensemble (tools/ClimateAssesment), writing the AR6-diagnostics GSAT to output/climate\_assessment/.
\item[set run\_visualization=off]making the standard analysis figures (tools/Visualization).
\item[set Feasibility=off]feasibility analysis of the scenarios (tools/Feasibility).
\item[set Burke=off]Burke climate economic-impact calculator (tools/BurkeCIcalculator), run as a submodule. It reads the AIM/Hub global-mean temperature (preferring the probabilistic climate-assessment GSAT and falling back to the deterministic MAGICC gdx) and writes country and regional (R5/R10/R17) GDP-per-capita impacts, figures (world map, major-economy bars, regional time series), and a standalone IAMC-format file to output/CI/Burke/. It is not merged with the AIM/Hub IAMC database.
